...
public:
  ...
  bool active() const
  {
    const bool any_errors = inbound_stream_.reader().has_error() or sender_.writer().has_error();
    const bool sender_active = sender_.sequence_numbers_in_flight() or not sender_.reader().is_finished();
    const bool receiver_active = not inbound_stream_.writer().is_closed();
  ...
  void receive( TCPMessage msg, const TransmitFunction& transmit )
  {
    ...
    receiver_.receive( std::move( msg.sender.get_mut() ), reassembler_, inbound_stream_.writer() );
    ...
    if ( inbound_stream_.writer().is_closed() and not std::as_const( sender_ ).reader().is_finished() ) {
...
private:
  ...
  ByteStream inbound_stream_ { cfg_.recv_capacity };

  bool need_send_ {};

  void send( const TCPSenderMessage& sender_message, const TransmitFunction& transmit )
  {
    transmit( { .sender = borrow( sender_message ), .receiver = receiver_.send( inbound_stream_.writer() ) } );
    need_send_ = false;
  }
