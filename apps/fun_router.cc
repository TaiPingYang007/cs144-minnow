...
string concatenate( vector<Buffer>&& buffers )
{
  string ret;
...
string summary( const EthernetFrame& frame )
{
  ...
  for ( const auto& buffer : frame.payload ) {
    payload_size += buffer.size();
  }

  return "Ethernet frame from " + to_string( frame.header.src ) + " to " + to_string( frame.header.dst )
         + ", type=" + to_string( frame.header.type ) + ", payload_size=" + to_string( payload_size );
}
...
struct Ethernet_over_UDP : public NetworkInterface::OutputPort
{
  ...
  {
    socket_.sendto( physical_dest_, concatenate( serialize( x ) ) );
  }
};
                    ...
                    vector<shared_ptr<NetworkInterface>>& interfaces )
{
  ...
  for ( size_t arg_idx = 1; arg_idx < args.size(); ++arg_idx ) { // ignore argv[0] (the name of the program)
    const string_view config { args[arg_idx] };
...
void program_body( const span<char*>& args )
{
  ...
  for ( size_t i = 0; i < ports.size(); ++i ) {
    ...
    event_loop.add_rule( category_id, port.socket_, Direction::In, [&] {
      string incoming_datagram {};
      Address incoming_address { "0" };
      ...
      EthernetFrame frame;
      vector<Buffer> frame_payload { move( incoming_datagram ) };
      if ( not parse( frame, frame_payload ) ) {
